...
def genWordPassword(bits):
    ...
    for i in range(0,words):
        rand = randint(1, len(wordList) * 10) % len(wordList) #Avoid modulo bias by having max value be a multiple of the length of the wordlist
        password += wordList[rand]
        password += " "
    password = password[:-1]
    return library.mainLib.sortPasswordString(password)
...
if args.verify is not None:
    ...
    if library.mainLib.genWordChecksum(args.password) == args.checksum:
    ...
    if int(bits) < int(10):
        ...
        print password
        print library.mainLib.genWordChecksum(password)
